            ...
            <div id="datamhs">
                ...
                <div class="d-flex justify-content-center">
                    <table id="tabelmhs">
                        ...
                        $query = "SELECT * FROM tb_skripsi JOIN tb_mhs where tb_skripsi.nim = tb_mhs.nim ORDER BY tb_skripsi.verif_skripsi";
                        $connect = mysqli_query($conn, $query);
                        $no = 1;

                        if (isset($_GET['cari_mhs'])){
                            $nama_mhs = $_GET['nama_mhs'];
                            $query = "SELECT * FROM tb_skripsi JOIN tb_mhs where tb_skripsi.nim = tb_mhs.nim && tb_mhs.nama LIKE '%".$nama_mhs."%' ORDER BY tb_skripsi.verif_skripsi";
                            $connect = mysqli_query($conn, $query);
                            $no = 1;
                        }
                    ...
                    while ($data = $connect->fetch_object()) {
                        ...
                        <td id="table1"><button type="button" class="btn btn-primary"
                                onclick="location.href = 'scanSkripsi.php?nim=<?php echo $data->nim ?>'">Lihat
